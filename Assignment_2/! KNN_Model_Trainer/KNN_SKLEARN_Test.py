...
Y_Pred = grid_search_KNearest.predict(X_Test_Array)

# conversions to make scores work correctly
lb = LabelBinarizer()
lb.fit(Y_Test_Array)
Y_Test_Array_Bin = lb.transform(Y_Test_Array)
Y_pred_Bin = lb.transform(Y_Pred)
roc_auc = roc_auc_score(Y_Test_Array_Bin, Y_pred_Bin, multi_class='ovr', average='weighted')
precision = precision_score(y_true=Y_Test_Array, y_pred=Y_Pred, average='weighted')
...
print('Accuracy Score - KNN:', metrics.accuracy_score(Y_Test_Array, Y_Pred))  
print('Precision - KNN:', precision)
print('F1 Score - KNN:', metrics.f1_score(Y_Test_Array, Y_Pred, average='weighted')) 
print('Recall - KNN:', metrics.recall_score(Y_Test_Array, Y_Pred, average='weighted'))
